...
### Worked on other Sample data to show the Working of k-mean Cluster Algorithm:
...
# ### Lets Implement K-means Clustering:
...
# Correlation matrix
corrMatrix =df.corr(method='spearman', min_periods=1)
corrMatrix.style.background_gradient(cmap = 'rocket_r')
...
ss = StandardScaler()
data = ss.fit_transform(cat_data)


# In[21]:


inertias = pd.Series(name='inertia')
for n_clusters in range(2,20):
    km = KMeans(n_clusters=n_clusters)
    km.fit(data)
    data_pred = km.predict(data)
    inertias[str(n_clusters)] = km.inertia_


# In[22]:


inertias.plot(title='elbow');
...
sample["Predicted"] = data_pred
sample.to_csv('submission_kmean.csv',index=False)
...
# ##  Implemetation of DBSCAN clustering algorithm

# In[32]:


#importing DBSCAN from sklearn lib
...
pca = PCA(n_components = 2)
data_principal = pca.fit_transform(normalized_data)
...
data_principal.head(10).style.background_gradient(cmap = 'rocket_r')
...
db_default = DBSCAN(eps =10, min_samples = 5).fit(data_principal)
...
#  
#  **If you like this my work ..Kindly upvote and share to others....**
# #  Thanks for visiting my notebooks
